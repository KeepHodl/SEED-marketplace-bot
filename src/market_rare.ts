...
import { API_URL_RARE, BUY_URL, HEADERS, SELL_URL } from './api';

let errorCount = 0;

const checkMarketRare = async () => {
  try {
    const response = await fetch(API_URL_RARE, { headers: HEADERS });
    if (!response.ok) {
      errorCount++;
...
export default checkMarketRare;
