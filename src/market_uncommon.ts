...
import { API_URL_UNCOMMON, BUY_URL, HEADERS, SELL_URL } from './api';

let errorCount = 0;

const checkMarketUncommon = async () => {
  try {
    const response = await fetch(API_URL_UNCOMMON, { headers: HEADERS });
    if (!response.ok) {
      errorCount++;
...
export default checkMarketUncommon;
