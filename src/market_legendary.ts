...
import { API_URL_EPIC, BUY_URL, HEADERS, SELL_URL } from './api';

let errorCount = 0;

const checkMarketEpic = async () => {
  try {
    const response = await fetch(API_URL_EPIC, { headers: HEADERS });
    if (!response.ok) {
      errorCount++;
...
export default checkMarketEpic;
